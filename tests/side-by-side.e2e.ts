...
test('has text', async ({ page, webApp: auto }) => {
    ...
    await expect(auto.locator('title')).toHaveTextA(/Capacitor/, { useInnerText: true });;
});
...
test('has attribute', async ({ page, webApp: auto }) => {
    ...
    await expect(auto.locator('.button', { hasText: 'Read more' })).toHaveAttributeA('href', 'https://capacitorjs.com');
});
